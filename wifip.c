...
volatile unsigned char time_sensor,light_led_ss=0,use_bt,dk_app;
extern unsigned char use_touch,use_sensor,count_reset;
uint16_t count_1s = 0;
...
//		
			...
			// thoi gian dao trang thai led
		{
			...
			{		
				...
				if(timeout_config > 20)  // nhay trong 30 lan dem se thoat nhay led
				{
					modeconfig = 0;
					timeout_config = 0;
...
//		//dinh ky reset touch 1 lan
		...
		//	count_reset=0; // reset MCU
			count_reset_touch=0; //reset touch auto
			if(count_setup == 1)//config mode
			{
				...
				use_touch=1;
				use_sensor=0;
				modeconfig = 2; // che do cho nhay led luan phien
				count_setup = 0;
				}
				else
				{
		...
		//	count_reset=0; // reset MCU
			...
			if(count_setup == 1)//config mode
			{
				...
				use_touch=0;
				modeconfig = 3; // che do cho nhay led luan phien
				count_setup = 0;
				}
				else
				{
